...
#define GUID_USB_INTERFACE_STRING L"{A5DCBF10-6530-11D2-901F-00C04FB951ED}"
...
#define WSTR_LEN 512
	...
	HID_API_EXPORT hid_device* HID_API_CALL hid_open(unsigned short vendor_id, unsigned short product_id, const wchar_t* serial_number, int interface_number)
	{
		...
		while (cur_dev) {
			if (cur_dev->vendor_id == vendor_id &&
				cur_dev->product_id == product_id && cur_dev->interface_number == interface_number) {
				if (serial_number) {
	...
	int HID_API_EXPORT HID_API_CALL hid_get_input_report(hid_device* dev, const unsigned char* data, size_t length)
	{
