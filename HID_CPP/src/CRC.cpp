...
uint32_t CCRC::Crc32(const uint8_t* buf, uint32_t size)
{
    uint32_t i, crc;
    crc = 0xFFFFFFFF;
    for (i = 0; i < size; i++)
...
uint32_t CCRC::Crc(const uint8_t* buf, uint32_t size, CRCType type)
{
    uint32_t ret = -1;
    if (type == CRCType::crc_8) {
        ret = Crc8(buf, size);
    }
    else if (type == CRCType::crc_16) {
        ret = Crc16(buf, size);
    }
    else if (type == CRCType::crc_32) {
        ret = Crc32(buf, size);
    }
    return ret;
}

uint8_t CCRC::Crc8(const uint8_t* buf, uint32_t size)
{
    if (!buf) return 0;

    uint8_t uCrc = 0;
    while (size--)
        uCrc = g_Crc8Table[uCrc ^ (*buf++)];

    return uCrc ^ 0xff;
}
uint16_t CCRC::Crc16(const uint8_t* buf, uint32_t size)
{
